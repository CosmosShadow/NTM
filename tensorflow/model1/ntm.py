...
class NTM(object):
    def __init__(self, cell, sess,
        ...
        self.opt = tf.train.RMSPropOptimizer(self.lr, decay=self.decay, momentum=self.momentum)
        self.build_model(forward_only)


    def build_model(self, forward_only, is_copy=True):
        print("Build NTM model starts")
        with tf.variable_scope(self.scope):
            ...
            if not forward_only:
                for seq_length in xrange(self.min_length, self.max_length + 1):
                    ...
                    self.optims[seq_length] = self.opt.apply_gradients(zip(grads, self.params), global_step=self.global_step)

        self.saver = tf.train.Saver()
        print("Build NTM model finished")
