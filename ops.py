...
def Linear(input_, output_size, stddev=0.5, is_range=False, squeeze=False, name=None, reuse=None):
    with tf.variable_scope("Linear", reuse=reuse):
        ...
        w = tf.get_variable(w_name, [input_size, output_size], tf.float32, tf.random_normal_initializer(stddev=stddev))
        ...
        if is_range:
            ...
            b = tf.get_variable(b_name, [output_size], tf.float32, identity_initializer(tf.cast(range_, tf.float32)))
        else:
...
def binary_cross_entropy_with_logits(logits, targets, name=None):
    ...
    with ops.op_scope([logits, targets], name, "bce_loss") as name:
        ...
        return -math_ops.reduce_mean(logits * math_ops.log(targets + eps) + (1 - logits) * math_ops.log(1 - targets + eps))
